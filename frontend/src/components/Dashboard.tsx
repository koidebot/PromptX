...
const Dashboard: React.FC = () => {
  ...
  const fetchPromptHistory = async () => {
    try {
      const response = await fetch(`${BASE_URL}/prompt-history`, {
        headers: {
    ...
    try {
      ...
      const pollJob = async () => {
        ...
        if (result.status === "completed" && result.final_prompt) {
          ...
          setIsLoading(false);
          // Refresh prompt history after job completion
          fetchPromptHistory();
  ...
  const loadHistoryItem = (historyItem: PromptHistoryItem) => {
    setPrompt(historyItem.initial_prompt);
    setImprovedPrompt(historyItem.final_prompt);
  };
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-black text-gray-900 dark:text-white">
      ...
      <div className="flex-1 flex flex-col">
        ...
        {/* Main Content Area */}
        <main className="flex-grow px-6 lg:px-12 py-8 overflow-y-auto">
          {/* Hero Section */}
                ...
                <div className="flex justify-between items-center mb-6">
                  ...
                  {improvedPrompt && !isLoading && (
                    <button
                      ...
                      className="px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-lg transition-colors duration-200 flex items-center space-x-2"
                    >
